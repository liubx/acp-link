...
/// 处理 web_send_file MCP tool：复制文件到 .tmp/downloads/ 并通过 broadcast 通知 SSE 流
async fn handle_web_send_file(args: &Value) -> Result<Value, String> {
    ...
    // 保存到 cwd/.tmp/downloads/（使用时间戳避免冲突）
    let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    let upload_dir = cwd.join(".tmp/downloads");
    ...
    let url = format!("/.tmp/downloads/{}", urlencoding::encode(&final_name));
